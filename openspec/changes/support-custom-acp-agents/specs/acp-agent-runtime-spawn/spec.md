# acp-agent-runtime-spawn 规范（变更）

## Purpose

定义 ACP agent distribution 中 `args` / `env` 运行时元数据的共享类型与启动注入规则，确保 npx、uvx、binary、custom 四类分发在保持兼容的同时正确传递参数和环境变量。本次变更新增 custom 类型分发。
