# agent-status-cache 规范（变更）

## Purpose

定义 CLI agent 安装状态的批量检测、本地缓存、stale-while-revalidate 刷新与状态广播机制。本次变更将检测、缓存与广播范围从 Registry Agent 扩展到 Catalog Agent（Registry + Custom）。
