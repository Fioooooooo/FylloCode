...
- 点击任意文件变更后打开一个与现有文件预览一致宽度的 window-level Slideover，以 Nuxt UI Accordion 展示该 turn 的文件列表；文件项默认全部折叠，用户可任意展开多个，每个文件项使用项目锁定的 `stream-monaco@0.0.46` `createDiffEditor()` 显示独立只读 diff，并关闭 Monaco 自带的 `diffOverview`。Accordion content 不设置固定高度或最大高度，由 diff 内容自然撑开并由 Slideover 外层统一滚动；收起时只隐藏 content、不销毁 editor。新建文件使用空 original，删除文件使用空 modified。
...
- 测试：工具状态、turn diff 聚合、overlay 生命周期、Monaco diff 参数、主题、`diffOverview` 关闭、Accordion 默认全折叠与多项展开、折叠 content 保留、重复折叠重开以及实时/历史一致性。
