...
#### Scenario: user_message chunk 不触发组装

- **WHEN** composable 收到 `{ kind: "user_message", message }` chunk
- **THEN** 将 `message` 原样追加到消息容器
- **AND** 清空 `activeAssistantId`（下一个 assistant 事件将新建 message）
