...
事件栏 SHALL 支持展示当前会话的 Agent 行动清单事件、proposal 入口事件，以及当前 Chat 主会话消息中仍未处理的 Fyllo action 事件。待处理 Fyllo action 事件 SHALL 从当前 `activeSession.messages` 与 `activeSession.actionStates` 派生，不新增 IPC，不新增持久化字段，不读取 Apply / Archive 运行日志。

事件栏无可展示事件时 SHALL 不渲染可见容器，避免占用聊天主区域空间。

#### Scenario: 活跃会话存在 agentAgenda 时显示事件栏

- **WHEN** 当前为已建立 session
- **AND** `activeSession.agentAgenda` 含至少 1 条行动清单项
...
- **WHEN** 当前为已建立 session
- **AND** `activeSession.agentAgenda` 为 `undefined` 或空数组
...
- **WHEN** 当前 session 存在 `agentAgenda`
- **THEN** 事件栏显示在消息列表区域右侧
...
系统 SHALL 为事件栏提供手动收起和展开控制。该控制 SHALL 是 Chat 主区域的局部布局控制，不得放入 `ChatPromptPanel`、全局 header 或 `ChatAgentAgendaPanel` 内部。
...
手动收起状态 SHALL 只作为 renderer 局部 UI 状态维护，不持久化到 session meta，不新增 IPC，不影响 `Session.agentAgenda` 数据。

#### Scenario: 用户收起事件栏

- **WHEN** 当前 session 存在 `agentAgenda` 且事件栏处于展开状态
- **AND** 用户点击事件栏顶部靠近分隔线的收起按钮
- **THEN** 事件栏内容被收起
- **AND** Chat 主区域右侧边界显示展开 handle
- **AND** `Session.agentAgenda` 不被修改

#### Scenario: 用户重新展开事件栏

- **WHEN** 当前 session 存在 `agentAgenda` 且事件栏处于收起状态
- **AND** 用户点击右侧边界的展开 handle
- **THEN** 事件栏重新展开并显示行动清单事件卡片
