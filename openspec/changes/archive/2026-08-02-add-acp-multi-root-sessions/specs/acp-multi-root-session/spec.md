...
本提案启用 Workspace Chat 多根目录时，proposal apply/archive activation SHALL 继续只传 run 固定 owner 的 Folder root 或 registered worktree 作为 `cwd`，其 `additionalDirectories` SHALL 为空；不得因来源 Workspace 有多个成员而暴露其他 Folder paths。

#### Scenario: Multi-root Workspace proposal apply

- **WHEN** proposal 来源 Workspace 含多个可用 Folder且 apply run 已固定 owner worktree
