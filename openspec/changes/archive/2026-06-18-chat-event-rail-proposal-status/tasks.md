...
- [x] 1.1 在 `src/shared/types/channels.ts` 的 `ProposalChannels` 对象中新增 `statusChanged: "proposal:statusChanged"`。
  - 验收标准：`ProposalChannels.statusChanged` 存在且值为 `"proposal:statusChanged"`；`pnpm typecheck` 通过。

- [x] 1.2 在 `src/shared/types/proposal.ts` 中新增 `ProposalStatusChangedPayload` 类型：
...
- [x] 1.3 扩展 `ProposalStatusChangedPayload` 支持 removed 标记（可选字段）：
...
- [x] 2.1 在 `src/main/infra/proposal/openspec-reader.ts` 中新增 `resolveChangeDirAnywhere(projectPath: string, changeId: string): Promise<{ dir: string; archived: boolean; worktreePath?: string } | null>` 函数。
  - 必须查找以下路径（按顺序）：
...
- [x] 3.1 创建 `src/main/services/proposal/proposal-status-service.ts`。
...
- [x] 3.4 实现 `unwatchProposal` 与 `unwatchAll`：
...
- [x] 4.1 修改 `src/main/services/lineage/mcp-event-consumer.ts`：
...
- [x] 4.2 确保 `ensureLineageEventConsumer` 在 chat 页面生命周期内可用：
...
- [x] 5.1 在 `src/main/ipc/proposal.ts` 中新增 `setupProposalStatusBroadcast(mainWindow: BrowserWindow): void`：
  - 内部保存 `mainWindow` 引用。
...
- [x] 5.2 在 `src/main/bootstrap/index.ts` 中挂载 broadcast：
...
- [x] 6.1 在 `src/preload/api/proposal.ts` 中新增 `onStatusChanged`：
...
- [x] 6.2 在 `src/preload/index.d.ts` 的 `ProposalApi` 接口中补充 `onStatusChanged` 类型声明。
  - 验收标准：`pnpm typecheck` 通过。

## 7. Renderer Stores

- [x] 7.1 扩展 `src/renderer/src/stores/session.ts`：
...
- [x] 7.2 在 `useSessionStore` 初始化时订阅 proposal 状态推送：
...
- [x] 7.3 在切换 active session 或初始化时回填历史 proposals：
...
- [x] 8.1 创建 `src/renderer/src/components/chat/event/ChatProposalPanel.vue`：
  - Props：
    ```ts
...
- [x] 8.2 修改 `src/renderer/src/components/chat/event/ChatSessionEventRail.vue`：
...
- [x] 10.1 为 `resolveChangeDirAnywhere` 添加单元测试（`test/main/infra/proposal/openspec-reader.test.ts` 或新建文件）：
  - 覆盖 main active、main archive、worktree active、worktree archive、not found。
  - 验收标准：所有新增测试通过。

- [x] 10.2 为 `ProposalStatusService` 添加单元测试（`test/main/services/proposal/proposal-status-service.test.ts`）：
  - 覆盖 status 变化、archive 移动、删除事件、重复 watch。
  - 验收标准：所有新增测试通过。

- [x] 10.3 为 `ChatProposalPanel` 添加组件测试（`test/renderer/components/chat/event/ChatProposalPanel.test.ts`）：
  - 覆盖 draft 状态显示“开始实现”、applying 状态不显示归档、applying+done 状态显示归档。
  - 验收标准：所有新增测试通过。

## 11. Documentation and Guidelines

- [x] 11.1 更新 `guidelines/IPC.md`：
  - 在“主进程主动向渲染进程推送事件”章节中补充 `proposal:statusChanged` 作为新增示例。
  - 验收标准：文档中提及新 channel 及其用途。

- [x] 11.2 更新 `guidelines/RendererProcess.md`（如其中有 chat 组件或 store 相关章节）：
  - 简要说明 `useSessionStore.sessionProposals` 的用途。
  - 验收标准：文档与代码一致。

## 12. 重启后 Proposal Watcher 恢复与乐观更新

- [x] 12.1 新增 `proposal:watch` IPC channel：
  - 在 `src/shared/types/channels.ts` 的 `ProposalChannels` 中新增 `watch: "proposal:watch"`。
  - 在 `src/shared/schemas/ipc/proposal.ts` 中新增 `watchProposalInputSchema`。
  - 在 `src/preload/api/proposal.ts` 与 `src/renderer/src/api/proposal.ts` 中新增 `watch(input)` API。
  - 在 `src/main/ipc/proposal.ts` 中新增 handler，调用 `proposalStatusService.watchProposal(projectPath, changeId, sessionId)`。
  - 验收标准：`pnpm typecheck` 通过；renderer 可调用 `proposalApi.watch`。

- [x] 12.2 在历史 proposal 回填时补挂 watcher：
  - 修改 `src/renderer/src/stores/session.ts` 的 `backfillSessionProposals()`：回填 `sessionProposals` 后，对每个 matched proposal 调用 `proposalApi.watch()`。
  - 在 `subscribeProposalStatus()` 中收到未知 proposal 的推送时，也调用 `ensureProposalWatched()` 作为防御。
  - 验收标准：应用重启后进入已有 chat session，关联的 draft proposal 被重新 watch；点击“开始实现”后 `.openspec.yaml` 更新能触发 `proposal:statusChanged`。

- [x] 12.3 EventRail 点击“开始实现”本地乐观更新：
  - 修改 `src/renderer/src/components/chat/event/ChatProposalPanel.vue` 的 `startApply()`：在 `startRun()` 成功后立即调用 `sessionStore.upsertSessionProposal()` 将对应 proposal status 更新为 `"applying"`。
  - 验收标准：点击后 EventRail 中 proposal 状态立即变为 applying，无需等待主进程推送。
