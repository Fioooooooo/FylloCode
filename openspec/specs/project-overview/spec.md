...
`mergeCommitSha` SHALL 表示当前 Git 历史中引入 `openspec/changes/archive/<archivedChangeId>/.openspec.yaml` 的 commit hash，而不是 archive tool 最初创建的临时 hash。人为 rebase、amend 或 squash 改写历史后，下一次 overview 查询 SHALL 反映当前历史中可达的新 hash。
