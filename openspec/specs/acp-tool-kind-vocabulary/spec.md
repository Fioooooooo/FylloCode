# acp-tool-kind-vocabulary Specification

## Purpose

定义 ACP 工具 kind 从 Main mapper 保留到 Renderer 展示的 canonical 词汇、兼容回退、语义图标与 Activity 摘要映射，确保新增协议类型和未知未来类型在普通工具、Activity group 与历史消息中保持可预测的用户表现。
