...
- 路径基于 `src/main/infra/storage/project-paths.ts` 的 `sessionsDir(projectPath)` 派生
...
文件 IO SHALL 经由 `src/main/infra/storage/attachment-store.ts` 提供的统一 API（`saveAttachment(projectPath, sessionId, fileName, mimeType, base64) → { absolutePath, fileUri, name, mimeType }`、`removeSessionAttachments(projectPath, sessionId)`）。其他模块 MUST NOT 直接拼接路径写文件。
...
入参校验 SHALL 通过 `src/shared/schemas/ipc/chat.ts` 的 `saveAttachmentInputSchema`：
...
入参校验 SHALL 通过 `src/shared/schemas/ipc/chat.ts` 的 `readAttachmentDataUrlInputSchema`：

- `uri` 为非空字符串且必须以 `file://` 开头
- `mediaType` 为非空字符串且必须以 `image/` 开头
- 本次读取接口 SHALL NOT 校验文件大小上限

handler SHALL 在 `src/main/ipc/chat.ts` 注册，使用 `validate(readAttachmentDataUrlInputSchema, input)` 校验入参，并调用 `src/main/infra/storage/attachment-store.ts` 提供的读取 helper。读取 helper SHALL 使用 `fileURLToPath(uri)` 转换 URI，使用 `fs.readFile` 读取文件内容，并返回 `data:${mediaType};base64,${buffer.toString("base64")}`。读取失败 SHALL 通过现有 `wrapHandler` 错误归一化路径返回失败响应，不得让异常直接穿透到 renderer。

preload SHALL 在 `src/preload/api/chat.ts` 暴露 `readAttachmentDataUrl(uri, mediaType)`，renderer SHALL 通过 `src/renderer/src/api/chat.ts` 的 `chatApi.readAttachmentDataUrl(uri, mediaType)` 薄封装调用，不得直接访问 `window.api.chat.readAttachmentDataUrl`。
