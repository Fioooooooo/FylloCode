# workspace-presentation-terminology Specification

## Purpose

定义 Workspace 领域模型在用户界面中的统一呈现术语及映射边界，确保 Project、Workspace、项目目录与回收站等文案由对象语义决定，同时保持内部类型、API、存储结构和 Agent 契约不变。
