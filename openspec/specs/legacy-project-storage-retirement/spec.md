...
Settlement pending 或 latest attempt failed 时，bootstrap SHALL 在启动 bundled MCP、IPC、workflow、窗口与 Agent warmup 前停止，并 SHALL 记录 settlement migration ID、最新错误与日志目录。原生失败 UI SHALL 说明未获授权数据不会被删除、下次启动会重试 settlement，并只提供打开日志目录或退出应用。

#### Scenario: Settlement latest attempt failed

- **WHEN** bootstrap gate 读取到 settlement 的最新 record 为 failed
- **THEN** 原生对话框 SHALL 显示 settlement ID、最新错误和日志路径
