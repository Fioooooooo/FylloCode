...
describe("shouldSuppressDuringReplay", () => {
  it("does not suppress available_commands_update", () => {
    expect(shouldSuppressDuringReplay({ type: "available_commands_update", commands: [] })).toBe(
      false
    );
  });

  it("does not suppress session_info_update", () => {
    expect(shouldSuppressDuringReplay({ type: "session_info_update", title: "x" })).toBe(false);
  });

  it("does not suppress config_options_update", () => {
    expect(shouldSuppressDuringReplay({ type: "config_options_update", options: [] })).toBe(false);
  });

  it("suppresses text_delta", () => {
    expect(shouldSuppressDuringReplay({ type: "text_delta", text: "hi" })).toBe(true);
  });
