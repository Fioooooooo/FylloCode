...
- 测试统一放在 `test/renderer/src/` 下，按 `src/renderer/src/` 镜像组织，不与生产代码并置
...
1. 与组件分离，单独放在 `src/renderer/src/composables/` 目录
2. 测试文件放在 `test/renderer/src/composables/`
3. 重点测试 **状态流转、副作用时机、边界条件**
4. 涉及 IPC 的 composable，优先 mock `window.api`

## 纯函数 / Utils 测试

- 放在 `src/renderer/src/utils/` 目录
- 测试文件放在 `test/renderer/src/utils/`
