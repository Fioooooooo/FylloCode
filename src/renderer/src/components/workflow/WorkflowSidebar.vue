  ...
  <div class="flex-1 overflow-y-auto py-2">
    <section>
      ...
      >
        <button
          type="button"
          class="w-full rounded-md px-3 py-2.5 pr-10 cursor-pointer hover:bg-elevated transition-colors duration-150 text-left"
          :class="
            selectedTemplateId === template.id ? 'bg-primary/15 text-primary' : 'bg-transparent'
          "
    ...
    <section class="mt-3">
      ...
      <button
        ...
        :key="template.id"
        type="button"
        class="mx-2 w-[calc(100%-1rem)] rounded-md px-3 py-2.5 cursor-pointer text-left transition-colors hover:bg-elevated"
        :class="
          selectedTemplateId === template.id ? 'bg-primary/15 text-primary' : 'bg-transparent'
        "
        @click="emit('select', template.id)"
