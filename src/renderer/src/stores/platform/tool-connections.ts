...
import { categories, tools } from "./tool-connections-config";
...
export const useIntegrationStore = defineStore("integration", () => {
  ...
  async function loadConnections(): Promise<void> {
    const res = await providersApi.getConnections();
  ...
  async function connectTool(
    ...
    const res = await providersApi.connect(toolId, credentials);
  ...
  async function disconnectTool(toolId: string): Promise<void> {
    await providersApi.disconnect(toolId);
    await loadConnections();
  }
