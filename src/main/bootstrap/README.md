...
`shutdown.ts#SHUTDOWN_PHASES` 是退出任务、owner、API 与 PID ownership 的可执行清单；本表只解释规则，不另建第二套顺序。`snapshot-and-hide` 在等待 protected migration 前同步执行，保证用户立即看到应用退出；随后 quiesce 中的 timer、watcher、session、grant 与 installer 任务并行，terminate 中 ACP、bundled MCP 与 auxiliary child 并行。
