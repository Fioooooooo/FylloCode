...
2. Read the current file from `state.knowledgeRoot`, revise the full markdown directly on disk, and preserve frontmatter fields that are still valid. Repository evidence must retain an explicit authorized `folderId`; do not replace a missing owner with primary or another Folder that happens to contain the same relative path.
