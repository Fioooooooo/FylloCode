...
3. Verify supporting files or references before writing final entry fields. Copy each repository evidence owner's `folderId` from the current Workspace state; do not infer ownership from primary or from a matching relative path.
