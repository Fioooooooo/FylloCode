...
export function registerTools(server: McpServer, rpc: SpawnRpcClient): void {
  ...
  server.registerTool(
    ...
    {
      description:
        "Delegate one focused task to an installed ACP Agent, or continue a spawned Session. Set background=true to return once Main has durably accepted and dispatched the turn; poll check_session_status and read the completed result with responseId + read_response. Background turns have no absolute runtime limit, but remain subject to inactivity cancellation and active-turn capacity. Split parallel work into non-overlapping file scopes because spawned Agents share the parent Workspace directories.",
