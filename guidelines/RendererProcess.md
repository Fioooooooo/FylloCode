...
- MUST 让 renderer 侧 knowledge capture/review 走既有会话和 action 管线：`knowledge.flag` pending 状态从当前已加载 assistant messages 中解析并展示在 EventRail，EventRail 只负责展示和定位 pending flags，不提供 capture 操作按钮；inline ActionShell 确认由 `knowledge.flag` handler 编排 capture message，handler 负责检查 chat store `chatStatus` 并调用 chat store `sendMessage`，chat store 不承载 knowledge 专用文案编排。capture 成功后，handler 应让同批 pending `knowledge.flag` action ids 通过现有 Fyllo Action state 机制一并标记完成，不新增 knowledge 专用 session meta 投影。capture message 的第一个 text part 应是隐藏 `<system-reminder>`，第二个 text part 应是用户可见的自然语言请求。`knowledge.review` handler 打开 review slideover，slideover 通过 `insight:knowledge` API 读取/保存完整 markdown 原文。renderer 组件不得直接写 knowledge 文件，也不得把 knowledge review 放入 lineage store。证据：`src/renderer/src/utils/fyllo-action-rail.ts`、`src/renderer/src/composables/fyllo-action-handlers/knowledge-flag.ts`、`src/renderer/src/components/shared/markstream/FylloActionShell.vue`、`src/renderer/src/stores/session/chat.ts`、`src/renderer/src/composables/useKnowledgeReviewSlideover.ts`、`src/renderer/src/stores/insight/knowledge.ts`。
