...
- MUST 让 `src/main/index.ts` 作为最小单实例门：该入口只静态依赖 Electron `app`，必须在加载 `@main/bootstrap` 前同步取得 `app.requestSingleInstanceLock()`；未取得锁的进程只请求退出，不得加载 bootstrap 或启动 app-data writer。持锁实例必须在动态加载 bootstrap 前监听 `second-instance`，并通过 `startApp()` 返回的 `PrimaryInstanceController` 请求窗口注意力；controller 必须等 migration、IPC/event 注册和首窗创建完成后再复用 `ProjectWindowManager.focusLastActiveWindow()` / `openLauncherWindow()`，不得绕过启动顺序直接操作 `BrowserWindow`。证据：`src/main/index.ts`、`src/main/bootstrap/index.ts`、`test/main/index.spec.ts`、`test/main/bootstrap/index.spec.ts`、`openspec/specs/single-instance-startup/spec.md`。
...
- MUST 将项目级 durable knowledge 持久化放在 app data 的 `projectDir(projectPath)/knowledge` 下，由 `src/main/infra/storage/project-paths.ts` 提供路径约定，并复用 `src/mcp-servers/fyllo-cortex/src/utils/knowledge.ts` 的扫描/序列化能力。Raw markdown review 的读取/保存、scanner-backed browser index 和单条删除 IPC 都属于 `insight:knowledge` area，与 `insight:lineage` 平级；browser index 必须复用 scanner 隔离 parse errors，删除只能接受 `knowledgeEntryNameSchema` 校验后的 name 并限制在当前项目 knowledge 目录，不得暴露任意 path 删除。Main 不在 `knowledge.review` 确认阶段执行 capture/update/retire operation。证据：`src/shared/ipc/insight/knowledge.channels.ts`、`src/shared/ipc/insight/knowledge.schemas.ts`、`src/main/ipc/insight/knowledge.ts`、`src/main/services/insight/knowledge/knowledge-document-service.ts`。
...
- MUST 让 `src/main/infra/mcp/bundled-mcp-host.ts` 统一拥有 bundled MCP HTTP proxy、后端子进程、`name -> backendPort` 内存路由、应用级 token、有限重启和 lifecycle 清理；ACP chat/probe 只能通过 `resolveBundledMcpServers()` 获取 HTTP/stdio transport spec，不得直接读取后端端口。renderer window 启动不得等待 MCP readiness，首次门控只发生在 ACP `newSession` 前。`src/mcp-servers/**` 继续保持不依赖 Electron 或 `@main/*`，HTTP 请求级项目/session context 必须经 shared `AsyncLocalStorage` getter 读取，不得按请求修改 `process.env`。证据：`src/main/infra/mcp/bundled-mcp-host.ts`、`src/main/infra/mcp/bundled-mcp-servers.ts`、`src/main/bootstrap/index.ts`、`src/main/services/session/chat/acp-session.ts`、`src/main/services/session/chat/session-probe-service.ts`、`src/mcp-servers/shared/request-context.ts`。
