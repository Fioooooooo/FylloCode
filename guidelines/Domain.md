...
- MUST: ACP 协议边界的 `sessionUpdate === "plan"` 在 FylloCode 内部映射为 `agenda_update`，共享类型使用 `AgendaEntry`，渲染层会话内存字段使用 `Session.agentAgenda`。不得把该运行时行动清单与 FylloCode 自有 `create-plan` / `plan.create` / lineage plan 链路混用。
