...
- MUST 通过 `renderer-features/boundaries` 自动约束 `src/renderer/src/features/<feature>/`：feature 外部只能导入根公共入口或标准 `/integration` 入口；四层依赖只能从外向内；model 不得依赖 Vue、Pinia、Electron、renderer infrastructure 或其他 feature；application 不得依赖 UI、SFC 或其他 feature；UI 不得直接依赖 renderer API wrapper 或其他 feature 的 integration；公共入口不得使用 `export *`。规则 MUST 从通用目录语义和 import pattern 推导，不得包含具体 feature、组件或迁移文件的白名单、ignore 或按文件例外，确保新增 feature 无需修改 ESLint 配置即可获得相同约束。证据：`scripts/eslint-rules/renderer-feature-boundaries.mjs`、`eslint.config.mjs`、`test/main/scripts/renderer-feature-boundaries.spec.mjs`；完整架构规则见 `guidelines/RendererFeatures.md`。
- MUST 通过 `renderer-terminology/no-internal-user-terms` 阻止 renderer 用户文案直接使用内部 Folder/Collection 展示名。规则必须检查 Vue template 与脚本字符串中的 UI 文案 sink，同时忽略 identifier、lowercase enum、module specifier 和 data attribute；规则依据禁止的内部呈现语义工作，不得维护完整允许句子或文件 allowlist。确属 Agent-facing、协议或诊断的字符串只能使用带理由的行级 ESLint disable 声明排除。Cleanup state、合法但可能误用的 `Workspace` 和动态 kind 呈现继续由 `workspace-presentation` helper 类型边界与 Project/Workspace 代表性测试约束。证据：`scripts/eslint-rules/renderer-user-terminology.mjs`、`eslint.config.mjs`、`test/main/scripts/renderer-user-terminology.spec.mjs`、`test/renderer/src/utils/workspace-presentation.spec.ts`；完整文案规则见 `guidelines/UiDesign.md`。
