...
- MUST 为 Workflow Engine 保持 domain → service/runner → IPC/MCP → lifecycle 的分层测试：`test/main/domain/automation/workflow/**` 只测试 v2 parser、capability preflight 和纯 state machine；`test/main/services/automation/workflow/**` 测试唯一 Engine、serialized Run、fresh Agent/exec Action、RPC bridge；`test/main/infra/storage/workflow-*.spec.ts` 测试 Workspace path、atomic snapshot、transcript/log 和 legacy inert；不得用集成测试掩盖 domain contract 缺少单元覆盖。证据：`src/main/domain/automation/workflow/**`、`src/main/services/automation/workflow/**`、`src/main/infra/storage/workflow-*.ts`、对应 test 目录。
- MUST 为 Workflow Run IPC 和 bundled MCP 分别测试 owner boundary 与 transport boundary：`test/main/ipc/automation/workflow-run*.spec.ts` 覆盖 Workspace/parent/run authorization、wake payload、debounce 和 shutdown 后拒绝；`test/main/infra/mcp/bundled-mcp-host.spec.ts`、registry/activation tests 覆盖 server-neutral codec、fyllo-spawn 回归、HTTP-only 与 fresh allowlist；`test/main/services/automation/workflow/workflow-rpc-bridge.spec.ts` 和 `test/mcp-servers/fyllo-workflow/**` 覆盖 trusted caller、schema strictness、rejected result 和 request cancellation。
- MUST 为 Renderer Workflow Run 保持与 spawned Session 分离的回归测试：`test/renderer/src/stores/automation/workflow-run.spec.ts` 覆盖 owner-keyed list/detail、interest ref-count、wake 无 interest 不 pull、generation 丢弃迟到响应和 approve/reject；`test/renderer/src/features/workflow-run-inspector/**` 覆盖纯 projection、ActivityEntry/detail UI、fresh transcript identity；`test/renderer/src/components/chat-background-activity-bar.spec.ts` 覆盖 WorkflowRunActivityEntry 与 SpawnedSessionActivityEntry 平级且 spawned count 不被改变。Workflow definition CRUD 继续由 `test/renderer/src/stores/automation/workflow.spec.ts` 与 workflow page tests 覆盖，不能把两种状态合并到一个 store fixture。
...
- MUST 将 app startup/shutdown 顺序测试放在 `test/main/bootstrap/`：覆盖 first-visible barrier 前无重型工作、同窗 handoff、protected migration、phase 串并行、单一绝对 deadline、force pending/失败、重复 quit 与 Windows session-end。Workflow Engine 必须在 bundled MCP readiness 后再 reconcile，在 quiesce/settle 阶段拒绝新 trigger、取消 handles、尽力落盘并在 ACP process pool/auxiliary process terminate 前完成 bounded dispose/force；parent deletion 的 cancellation ordering 放在对应 session service 测试。资源 owner 的 PID/process-group 和 graceful/force 行为放在对应 infra/service 镜像目录；`lifecycle-boundaries.spec.ts` 必须扫描并禁止 service/infra 通过 import side effect 注册全局顺序。Renderer 的 critical/background、无 Welcome flash、static startup 和 reduced-motion 放在 `test/renderer/src/`。证据：`test/main/bootstrap/runtime.spec.ts`、`test/main/bootstrap/shutdown.spec.ts`、`test/main/bootstrap/lifecycle.spec.ts`、`test/main/bootstrap/lifecycle-boundaries.spec.ts`、`test/main/services/session/chat/chat-service.spec.ts`、`test/main/infra/process/acp-process-pool.spec.ts`、`test/main/infra/mcp/bundled-mcp-host.spec.ts`、`test/renderer/src/app-startup.spec.ts`、`test/renderer/src/startup-page.spec.ts`。
