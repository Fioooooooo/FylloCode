...
- MUST 将多窗口主进程行为测试放在对应 main 镜像目录：窗口生命周期和 `ProjectWindowManager` 放在 `test/main/bootstrap/`，窗口 IPC 放在 `test/main/ipc/`，窗口状态存储放在 `test/main/infra/storage/`。证据：`test/main/bootstrap/project-window-manager.spec.ts`、`test/main/ipc/window.spec.ts`、`test/main/infra/storage/window-state-store.spec.ts`。
- SHOULD 在 renderer store/component 测试中显式覆盖 launcher context、project context、`bound-current` 与 `created` / `focused-existing` 打开结果，避免组件绕过 window API 直接修改 `currentProject`。证据：`test/renderer/src/stores/project.spec.ts`、`test/renderer/src/components/app-header.spec.ts`、`test/renderer/src/components/welcome-view.spec.ts`。
