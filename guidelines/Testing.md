...
- 涉及文件系统写入的 `electron/main` 测试，优先使用
  `@main/__tests__/test-temp-root` 的 `createTestTempRoot()` 创建唯一临时目录；其他测试树如需自建临时目录，必须通过 Node.js 的 `tmpdir()` + `mkdtempSync()` 创建
- 不要硬编码 `/private/tmp`、`/var` 等依赖特定系统且在 GitHub Actions Ubuntu runner
  上可能无权限的路径
