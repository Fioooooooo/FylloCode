...
| **规则自进化**   | `fyllo-cortex` 目前提供 `guidelines` 工具，在每次任务后自动更新项目规范，让 Agent 始终遵循最新约定 |
...
- **持续演进的 guidelines**（来自 `fyllo-cortex`，在每次任务后自动更新）
...
目前通过 `fyllo-cortex.guidelines` 工具实现这一点：在 Chat 与 Proposal 过程中，Agent 会考虑是否需要更新 guidelines，
