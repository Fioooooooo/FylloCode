...
- `title`：若原始 title 命中 `mcp__` 格式，用归一后的 `server/tool` 作为可读标题；否则保留 ACP 原始 title（原生工具的具体命令/路径不受影响）。
...
- 工具主文案沿用既有 `title ?? toolName`：MCP 工具显示 `server/tool`，原生工具显示具体命令/路径；
...
1. claude-acp MCP 工具 `mcp__tavily__tavily_search` 得到 `toolName: "tavily/tavily_search"`，title 同为可读 `tavily/tavily_search`。
