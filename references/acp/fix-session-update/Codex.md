...
Codex MCP tool 根据结构化 `rawInput.{server,tool}` 归一为 `server/tool`。除下述文件编辑特例外，`title` 保留 ACP 原始人类可读值；update 自带的新 title 应更新已有卡片。
...
- Codex 文件工具直接显示 mapper 从结构化 diff 生成的 `Create/Edit <filename>` title；
...
4. Codex MCP 工具得到 `toolName: "server/tool"`，同时保留 ACP title。
