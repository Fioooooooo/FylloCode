...
除了从附件菜单选择文件，也可以把剪贴板图片直接粘贴到输入区，或把图片和普通文件拖放到 Chat 输入面板。粘贴不会阻止浏览器插入剪贴板中的文本，因此文字与图片可以一起进入草稿；拖放只接管文件，文本拖动仍使用系统原生行为。首版不读取目录，拖入目录时会明确反馈并继续接收同批次中受支持的文件。

FylloCode 会按当前 Agent 的图片和嵌入上下文能力检查每个附件。混合批次只保留受支持的项目，并用一次汇总反馈说明被拒绝的图片、文件或目录；被拒绝的项目不会创建预览、附件记录或 Session。
...
FylloCode 会识别 ACP 1.3 的 read、write、edit、delete、move、search、execute、think、fetch 与 switch mode 等工具语义，并在直接工具和 Activity 摘要中使用一致的图标和分类。未知或未来新增的类型安全回退为通用工具，不会根据 Agent 身份猜测类别。
