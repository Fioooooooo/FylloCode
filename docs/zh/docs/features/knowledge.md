...
知识沉淀页面用于浏览、核查当前 Workspace 已沉淀的 knowledge 条目。条目正文以只读方式展示，条目的产生和更新由 Agent 在会话中通过 `fyllo-cortex` 的 `knowledge` 工具完成；不再需要的条目仍可从页面删除。Knowledge 属于 Workspace，不直接归属于某一个 Project。
...
Knowledge 不是聊天记录的摘要，而是“如果丢了，未来某次会话会为此付出代价”的事实：需要重新推导、重新翻查代码或文档，或者可能被理解错的信息。Agent 在会话中遵循一套判断标准（俗称 flag test）来识别这类信息，常见形态包括：
...
左侧列出当前 Workspace 的全部 knowledge 条目，按类型分组（`project`、`reference`、`feedback`）；右侧显示选中条目的正文。条目如果引用了仓库文件或其他证据，页面会保留其 Project/Folder 归属。条目上如果出现 `suspect` 或 `unknown` 状态标记，说明这条知识可能已经过期或来源存疑，建议在依赖它之前先核实。

条目可以被删除；删除操作不可撤销。

## 与 lineage 的关系

Knowledge 条目本身不直接挂在某一条 lineage 脉络上。它是跨 Project、跨任务、跨会话共享的 Workspace 级积累，不是某一次变更的产物。它和 [guidelines](/docs/features/guidelines) 一起构成 `fyllo-cortex` 提供给后续 Agent 会话的背景。仓库证据会保留所属 Project/Folder，具体的存储位置、索引方式和判断标准见 [fyllo-cortex 参考](/docs/reference/fyllo-cortex)。
