...
| [Workflow 编排](/docs/features/workflow) | 用 YAML 固化团队自己的执行阶段和 Agent 分工 |
| [ACP Agents](/docs/features/agents) | 安装、识别和管理支持 ACP 的 Coding Agent |
| [研发系统集成](/docs/features/integrations) | 连接云效等研发系统，把任务结果回写到现有工具链 |

## 推荐理解方式

先从一个 Task 开始，然后沿着 Chat、Proposal、Apply & Archive 看完整流程，这条流程上的每一步都由 [lineage](/docs/guide/lineage) 串联，并最终汇聚到项目概览。ACP Agents 和研发系统集成是支撑能力，Workflow 编排则用于把团队已经认可的流程固化下来。
